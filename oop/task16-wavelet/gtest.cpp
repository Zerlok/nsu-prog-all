...
TEST(DataAdapter, At)
{
	DataAdapter<double, doubles> adapter;
	for (size_t i = 0; i < len; ++i)
		EXPECT_EQ(data[i], adapter.at(data, i));
}


TEST(DataAdapter, Size)
{
	DataAdapter<double, doubles> adapter;
	doubles empty;

	EXPECT_EQ(len, adapter.size(data));
	EXPECT_EQ(empty.size(), adapter.size(empty));
}
...
TEST(DividingTransformator, EncodeDecodeIntegers)
{
	...
	doubles doubled_data = reset_type<double>(initial_data);
	...
	EXPECT_EQ(initial_data, reset_type<uchar>(decoded1));

	tor.set_transformation(new daub4double());
	doubles encoded2 = tor.apply_forward(doubled_data);
	doubles decoded2 = tor.apply_backward(encoded2);
	EXPECT_NEAR_VALUES(doubled_data, decoded2, 6);
	EXPECT_EQ(initial_data, reset_type<uchar>(decoded2));
