...
Week::Week()
	: _begin(), _end()
{
	_begin = (_begin.get_day() > _begin.get_weekday())
			? _begin - _begin.get_weekday()
			: _begin - (_begin.get_day() - 1);
...
{
	_end = _begin - 1;
	_begin = _end - (7 - _end.get_weekday());
...
{
	out << "("
		<< week._begin.get_weekday_name().substr(0, 3) << ") " << week._begin
		<< " -> ("
		<< week._end.get_weekday_name().substr(0, 3) << ") " << week._end
		<< "";
	return out;
}
